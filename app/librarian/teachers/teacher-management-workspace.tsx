    ...
    <article className={styles.detailCard}>
      ...
      {tab === "profile" ? editing ? (
        ...
        <div className={styles.profilePane}>
          ...
          <div className={styles.profileActions}><button type="button" onClick={() => setEditing(true)} disabled={!writesEnabled || busy}>Редагувати</button><a href="#teacher-access-title" title={`У блоці кодів нижче знайдіть ${teacher.fullName}`}>Код і доступ</a><button type="button" onClick={() => void changeStatus()} disabled={!writesEnabled || busy || (teacher.status === "active" && closeBlockers.length > 0)}>{teacher.status === "active" ? "Закрити картку" : "Поновити картку"}</button>{deletionAllowed ? <button className={styles.dangerButton} type="button" onClick={() => void remove()} disabled={!writesEnabled || busy}>Видалити порожню картку</button> : null}</div>
