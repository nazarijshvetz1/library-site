...
export default function ReaderPortal({telegram=false}:{telegram?:boolean}){
  const [profile,setProfile]=useState<ReaderProfile|null>(null),[section,setSection]=useState("books"),[loading,setLoading]=useState(true),[busy,setBusy]=useState(false),[error,setError]=useState(""),[invite,setInvite]=useState(""),[inviteName,setInviteName]=useState(""),[consent,setConsent]=useState(false),[bot,setBot]=useState<string|null>(null);
  const started=useRef(false),loginSection=()=>setSection("login");
  ...
  return <div className={s.shell}><header className={s.header}><a className={s.brand} href="/"><img src={READER_LOGO} alt="Лейбл бібліотеки"/><span><strong>Єдина бібліотека</strong><small>Міжнародний ліцей МАУП</small></span></a><div className={s.headerLinks}><a href={LIBRARIKA_CATALOG_URL} target="_blank" rel="noopener noreferrer">Каталог Librarika <ExternalLink size={15}/></a>{profile?<button className={s.quiet} onClick={()=>navigate("profile")}><UserRound size={16}/>Мій профіль</button>:<button className={s.secondary} onClick={loginSection}><LogIn size={15}/>Увійти</button>}</div></header>
