...
const kinds=[['genre','Категорії'],['author','Автори'],['publisher','Видавництва'],['tag','Теги'],['series','Серії'],['popular','Популярні']];
export default function PublicLiteratureCatalog(){
 ...
 const params=new URLSearchParams({view:entity?'entity':kind&&kind!=='popular'?'entities':'catalog',q:search,kind,id:entity,sort:kind==='popular'?'popular':'title'});
 ...
 {book?<><Feedback state={detail}/>{record&&<><h1>{record.title}</h1><div className={s.bookHero}><Cover book={record}/><div><p>{record.author}</p><PublicationLine book={record} onEntity={openEntity}/><BookPublicMeta book={record}/><p>{record.type} · {record.total} прим.</p></div></div><div className={s.chips}>{record.entities.map((item:Row)=><button key={item.id} onClick={()=>openEntity(item)}>{item.name} · {item.count}</button>)}</div><p className={s.body}>{record.annotation}</p>{record.websiteUrl&&<a className={s.externalBookLink} href={record.websiteUrl} target="_blank" rel="noopener noreferrer">Відкрити сторінку книжки ↗</a>}<a className={s.primary} href={login+'?tab=catalog&book='+encodeURIComponent(book)}>Увійти, щоб забронювати</a></>}</>:<><h1>{entity?list.raw?.entity?.name||'Каталог':kind?kinds.find(x=>x[0]===kind)?.[1]:'Каталог'}</h1>{!entity&&<label className={s.field}>Пошук<input value={query} onChange={e=>setQuery(e.target.value)} placeholder={kind?'Введи назву або ім’я':'Назва, автор або категорія'}/></label>}{!kind&&<div className={s.chips}>{kinds.map(([id,label])=><button key={id} onClick={()=>{setKind(id);setPage(1);setQuery('');}}>{label}</button>)}</div>}{entity&&list.raw?.entity?.metadata&&<div className={s.panel}>{Object.entries(list.raw.entity.metadata).map(([key,value])=><p className={s.body} key={key}>{String(value)}</p>)}</div>}
 <Feedback state={list}/><p className={s.small}>Знайдено: {list.data?.total??'…'}</p><div className={s.stack}>{list.data?.items.map(row=>kind&&kind!=='popular'&&!entity?<button className={s.secondary} style={{justifyContent:'space-between'}} key={row.id} onClick={()=>openEntity(row)}><strong>{row.name}</strong><span>{row.count} книг ›</span></button>:<BookRow key={row.id} book={row} onBook={setBook} onEntity={openEntity}><span className={s.badge}>Доступно: {row.available}</span></BookRow>)}</div>{list.data&&!list.data.total&&<p className={s.empty}>За цим запитом нічого не знайдено.</p>}<InfinitePager data={list.data} loading={list.loading} error={list.error} onNext={setPage} onRetry={list.retry}/></>}
 </main></div>;
}
