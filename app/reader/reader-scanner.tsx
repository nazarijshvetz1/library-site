...
export default function ReaderScanner({onDetected}:{onDetected:(code:string)=>void}){
  ...
  async function start(){
    ...
    try{
      ...
      const active=await reader.decodeFromStream(camera,video.current,(result,_error,current)=>{if(run!==generation.current){current.stop();return;}if(result){const code=result.getText();current.stop();stop();onDetected(code);}});
  ...
  return <><button className={styles.secondary} type="button" onClick={start} disabled={starting||open}><Camera size={17}/>{starting?"Відкриваємо…":"Сканувати книгу"}</button>{message&&<p className={styles.error} role="status">{message}</p>}
    {open&&<dialog ref={dialog} className={styles.dialog} onCancel={stop} onClose={stop}><div className={styles.dialogInner}><div className={styles.dialogTop}><div><p className={styles.eyebrow}>Камера лише для сканування</p><h2>Наведіть на код книги</h2></div><button className={styles.iconButton} onClick={stop} aria-label="Закрити сканер"><X size={20}/></button></div><p className={styles.muted}>ISBN відкриє видання; бібліотечний штрихкод або QR — конкретний примірник. Зображення камери не надсилається на сервер.</p><div className={styles.scannerVideo}><video ref={video} autoPlay muted playsInline/></div><div className={styles.actions}><button className={styles.secondary} onClick={stop}>Ввести код вручну</button></div></div></dialog>}
  </>;
}
