...
import { validateMaterialCreateInput } from "@/lib/library-write-validation";
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!access.writesEnabled) {
    return librarianError(
      503,
      "writes_disabled",
      "Додавання матеріалів тимчасово вимкнено адміністратором.",
      false,
    );
  }
  ...
  const validated = validateMaterialCreateInput(body.value);
  if (!validated.ok) {
    return librarianError(
      400,
      "validation_failed",
      "Перевірте обов’язкові поля нового матеріалу.",
      true,
      validated.fieldErrors,
    );
  }
  try {
    const result = await createMaterialDirect(
      user,
      validated.value,
