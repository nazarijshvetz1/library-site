...
import { validateLoanReturnInput } from "@/lib/library-write-validation";
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!access.writesEnabled) {
    return librarianError(
      503,
      "writes_disabled",
      "Повернення матеріалів тимчасово вимкнено адміністратором.",
      false,
    );
  }
  ...
  const validated = validateLoanReturnInput(body.value);
  if (!validated.ok) {
    return librarianError(
      400,
      "validation_failed",
      "Перевірте позиції та кількість повернення.",
      true,
      validated.fieldErrors,
    );
  }
  try {
    const result = await returnLoanItems(
      user,
      validated.value,
    ...
    return librarianError(
      503,
      "loan_return_unavailable",
      "Не вдалося зберегти повернення. Спробуйте ще раз.",
      true,
    );
  }
}
