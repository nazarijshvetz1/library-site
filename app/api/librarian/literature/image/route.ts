...
import { beginLibraryCommand,finishLibraryCommand,libraryCommand } from "@/lib/library-copy-store";
import { ReaderError,readerBatch,readerFail,requireChanged,type ReaderDatabase } from "@/lib/reader-core";
...
export async function GET(request:Request){const auth=await authorizeLibrarianApi();if(!auth.ok)return auth.response;try{const url=new URL(request.url),id=url.searchParams.get("id")||"",kind=url.searchParams.get("kind")||"edition",db=env.DB as unknown as ReaderDatabase;
 ...
 const object=await coverBucket()?.get(key);if(!object)return new Response(null,{status:404,headers});return new Response(object.body,{headers:{...headers,"Content-Type":["image/jpeg","image/png","image/webp"].includes(object.httpMetadata?.contentType||"")?object.httpMetadata!.contentType!:mime}});
 }catch{return new Response(null,{status:503,headers});}}
export async function POST(request:Request){const auth=await authorizeLibrarianApi();if(!auth.ok)return auth.response;if(!auth.value.access.writesEnabled||!auth.value.user.d1UserId)return librarianError(503,"writes_disabled","Запис вимкнено.",false);if(!isSameOriginRequest(request))return librarianError(403,"origin","Запит має надійти з цього сайту.",true);
 ...
 await bucket.put(key,bytes.buffer,{httpMetadata:{contentType:detected.contentType},customMetadata:{owner:id}});
 const now=new Date().toISOString(),result={id,version:expected+1},statements=[beginLibraryCommand(db,actor,String(input.requestId),"literature.image",command.hash,id,now)];
 if(kind==="edition")statements.push(db.prepare("UPDATE library_editions SET public_metadata_json=json_set(public_metadata_json,'$.literatureCoverKey',?,'$.literatureCoverMime',?),version=version+1,updated_at=? WHERE id=? AND version=? AND fund='literature'").bind(key,detected.contentType,now,id,expected),requireChanged(db,1));
 else statements.push(db.prepare("UPDATE library_readers SET version=version+1,updated_at=? WHERE id=? AND version=?").bind(now,id,expected),requireChanged(db,1),db.prepare("INSERT INTO reader_profiles(reader_id,display_name,photo_key,photo_mime,updated_at) VALUES(?,'Читач',?,?,?) ON CONFLICT(reader_id) DO UPDATE SET photo_key=excluded.photo_key,photo_mime=excluded.photo_mime,version=reader_profiles.version+1,updated_at=excluded.updated_at").bind(id,key,detected.contentType,now));
 statements.push(...finishLibraryCommand(db,actor,String(input.requestId),"literature.image",id,result,now));await readerBatch(db,statements);return librarianJson({success:true,result});
 }catch(error){return error instanceof ReaderError?librarianError(error.status,error.code,error.message,true):librarianError(400,"image","Не вдалося зберегти зображення. Перевірте файл і повторіть.",true);}}
