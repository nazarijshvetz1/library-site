...
import {
  ...
  returnClassLoanItems,
} from "@/lib/library-mutation-store";
import { validateClassLoanReturnInput } from "@/lib/library-write-validation";
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!access.writesEnabled) {
    return librarianError(
      503,
      "writes_disabled",
      "Повернення матеріалів від класів тимчасово вимкнено адміністратором.",
      false,
    );
  }
  ...
  const validated = validateClassLoanReturnInput(body.value);
  if (!validated.ok) {
    return librarianError(
      400,
      "validation_failed",
      "Перевірте видачу, її версію, позиції та кількість повернення.",
      true,
      validated.fieldErrors,
    );
  }
  try {
    const result = await returnClassLoanItems(
      user,
      validated.value,
    ...
    return librarianError(
      503,
      "class_loan_return_unavailable",
      "Не вдалося зберегти повернення від класу. Спробуйте ще раз.",
      true,
    );
  }
}
