...
export async function POST(request: Request, context: RouteContext): Promise<Response> {
  ...
  if (!access.writesEnabled) {
    return librarianError(503, "writes_disabled", "Закриття класів тимчасово вимкнено.", false);
  }
  ...
  const validated = validateClassYearCloseInput(body.value);
  if (!validated.ok) {
    return librarianError(400, "validation_failed", "Перевірте дані закриття класу.", true, validated.fieldErrors);
  }
  try {
    const result = await closeClassYearDirect(
      user,
      classYearId,
      validated.value,
    ...
    return librarianError(503, "class_year_close_unavailable", "Не вдалося закрити клас.", true);
  }
}
