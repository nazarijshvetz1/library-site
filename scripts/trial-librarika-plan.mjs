...
try{
  ...
  const before=captureTrialBaseline(db);
  const pendingMigrations=applyPendingTrialMigrations(db);
  ...
  const invariants={existingTableDataUnchanged:verifyTrialBaseline(db,before),existingTableDdlUnchanged:oldTables.every(table=>db.prepare("SELECT sql FROM sqlite_schema WHERE type='table' AND name=?").get(table.name)?.sql===table.sql),activeReaderAccess:db.prepare("SELECT count(*) AS n FROM library_readers WHERE access_status!='inactive' OR linked_teacher_user_id IS NOT NULL").get().n,publishedEditions:db.prepare("SELECT count(*) AS n FROM library_editions WHERE publication_state!='draft' OR material_id IS NOT NULL").get().n,registeredCopies:db.prepare("SELECT count(*) AS n FROM library_copies WHERE registration!='unreconciled'").get().n,openLoans:db.prepare("SELECT count(*) AS n FROM reader_circulations WHERE status IN ('issued','overdue')").get().n,foreignKeyErrors:db.prepare("PRAGMA foreign_key_check").all().length};
  ...
  const report={passed:true,counts,invariants,pendingMigrations,sourcePlanSha256:hash(fs.readFileSync(planPath)),recoverySha256:plan.recoverySha256,note:"Local trial only. Production import, catalog activation and stock reconciliation have NOT run."};
