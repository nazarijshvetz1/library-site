...
import {readLibraryBookActivity} from "./library-activity.ts";
...
export async function libraryReaderDashboard(db:ReaderDatabase){const stats=await db.prepare(`SELECT (SELECT COUNT(*) FROM library_editions) titles,(SELECT COUNT(*) FROM library_editions WHERE publication_state='published') published,(SELECT COUNT(*) FROM library_copies) copies,(SELECT COUNT(*) FROM library_readers) readers,(SELECT COUNT(*) FROM reader_circulations WHERE status IN ('issued','overdue')) issued,(SELECT COUNT(*) FROM reader_book_requests WHERE status IN ('requested','ready')) requested,(SELECT COUNT(*) FROM library_ratings WHERE review_state='pending') reviews,(SELECT COUNT(*) FROM reading_reports WHERE status='open') reports`).first();return {counts:stats,activity:await readLibraryBookActivity(db)};}
