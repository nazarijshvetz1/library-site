...
export async function listLibraryReaders(db:ReaderDatabase,url:URL){
  ...
  const rows=await db.prepare(`SELECT r.id,r.full_name,r.member_no,r.kind,r.status,r.access_status,r.version,r.source_group_label,r.linked_teacher_user_id,
    ...
    (SELECT COUNT(*) FROM reader_circulations l WHERE l.reader_id=r.id AND l.status IN ('issued','overdue')) AS loan_count,
...
export async function saveLibraryReader(db:ReaderDatabase,actor:LibraryActor,input:{requestId:string;id?:string;expectedVersion?:number;fullName:string;memberNo:string;kind:string;classYearId:string|null}){
  if(typeof input.fullName!=="string"||input.fullName.trim().length<3||input.fullName.length>180||typeof input.memberNo!=="string"||!input.memberNo.trim()||input.memberNo.length>50||!["student","teacher","staff","other","unclassified"].includes(input.kind)||(input.classYearId&&!readerResource(input.classYearId)))readerFail("reader_fields","Перевірте ім’я, номер квитка, категорію та клас.");
  if(input.kind!=="student"&&input.classYearId)readerFail("reader_class","Клас призначається лише учневі.");
  const command=await libraryCommand(db,actor,input.requestId,"library.reader.save",input);if(command.replayed)return command.replayed;
  const now=new Date().toISOString(),id=input.id||"READER-"+crypto.randomUUID(),result={id,version:input.id?Number(input.expectedVersion)+1:1},fullName=input.fullName.trim().replace(/\s+/g," ");
  const statements=[beginLibraryCommand(db,actor,input.requestId,"library.reader.save",command.hash,id,now)];
  if(input.id)statements.push(db.prepare("UPDATE library_readers SET full_name=?,sort_name=?,member_no=?,kind=?,version=version+1,updated_at=? WHERE id=? AND version=? AND (linked_teacher_user_id IS NULL OR kind=?)").bind(fullName,normalizeCatalogSearchText(fullName),input.memberNo.trim(),input.kind,now,id,Number(input.expectedVersion),input.kind),requireChanged(db,1));
  else statements.push(db.prepare("INSERT INTO library_readers(id,member_no,full_name,sort_name,kind,status,access_status,created_at,updated_at) VALUES(?,?,?,?,?,'active','inactive',?,?)").bind(id,input.memberNo.trim(),fullName,normalizeCatalogSearchText(fullName),input.kind,now,now));
  ...
  statements.push(...finishLibraryCommand(db,actor,input.requestId,"library.reader.save",id,result,now));await readerBatch(db,statements);return result;
}
...
export async function libraryReaderDashboard(db:ReaderDatabase){const stats=await db.prepare(`SELECT (SELECT COUNT(*) FROM library_editions) titles,(SELECT COUNT(*) FROM library_editions WHERE publication_state='published') published,(SELECT COUNT(*) FROM library_copies) copies,(SELECT COUNT(*) FROM library_readers) readers,(SELECT COUNT(*) FROM reader_circulations WHERE status IN ('issued','overdue')) issued,(SELECT COUNT(*) FROM reader_book_requests WHERE status IN ('requested','ready')) requested,(SELECT COUNT(*) FROM library_ratings WHERE review_state='pending') reviews,(SELECT COUNT(*) FROM reading_reports WHERE status='open') reports`).first();return stats;}
