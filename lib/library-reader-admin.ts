import {requestEvent,readerEvent} from './reader-events.ts';
...
export async function setReaderRequestStatus(db:ReaderDatabase,actor:LibraryActor,input:{requestId:string;id:string;expectedVersion:number;status:string}){
  ...
  await readerBatch(db,[beginLibraryCommand(db,actor,input.requestId,"library.reader.request",command.hash,input.id,now),db.prepare("UPDATE reader_book_requests SET status=?,version=version+1,updated_at=? WHERE id=? AND version=? AND status IN ('requested','ready') AND status!=?").bind(input.status,now,input.id,input.expectedVersion,input.status),requireChanged(db,1),requestEvent(db,input.id,now),...finishLibraryCommand(db,actor,input.requestId,"library.reader.request",input.id,result,now)]);return result;
}
