...
const relationRoles=(kind:string)=>kind==="author"?"x.role IN ('author','coauthor')":`x.role='${kind}'`;
const relationMatch="((n.kind='author' AND x.role IN ('author','coauthor')) OR (n.kind!='author' AND x.role=n.kind))";
const linkedField = (kind:string,field=kind) => `COALESCE((SELECT group_concat(name,', ') FROM (SELECT DISTINCT n.id,n.name FROM library_edition_entities x JOIN library_catalog_entities n ON n.id=x.entity_id WHERE x.edition_id=e.id AND ${relationRoles(kind)} ORDER BY n.name,n.id)),json_extract(e.public_metadata_json,'$.${field}'),'')`;
...
const bookProjection = `e.id,e.title,e.version,e.material_id,e.publication_state,e.created_at,${linkedMetadata} AS public_metadata_json,${libraryCoverSql} AS cover_url,(SELECT json_group_array(json_object('id',id,'name',name,'kind',kind,'role',role)) FROM (SELECT n.id,n.name,n.kind,x.role FROM library_edition_entities x JOIN library_catalog_entities n ON n.id=x.entity_id WHERE x.edition_id=e.id ORDER BY n.kind,n.name,n.id)) relations_json,
...
export async function literatureBooks(db: ReaderDatabase, url: URL) {
  ...
  for(const [param,kind,field] of [["author","author","author"],["category","genre","genre"],["publisher","publisher","publisher"],["tag","tag","tags"]]){
    const exactId=text(url.searchParams.get(param+"Id")||"",120);if(exactId){where+=` AND EXISTS(SELECT 1 FROM library_edition_entities x JOIN library_catalog_entities n ON n.id=x.entity_id WHERE x.edition_id=e.id AND n.kind=? AND x.entity_id=? AND ${relationMatch})`;bindings.push(kind,exactId);continue;}
  ...
  const country=text(url.searchParams.get("country")||"",100);if(country){where+=" AND EXISTS(SELECT 1 FROM library_edition_entities ce JOIN library_catalog_entities a ON a.id=ce.entity_id WHERE ce.edition_id=e.id AND a.kind='author' AND ce.role IN ('author','coauthor') AND json_extract(a.public_metadata_json,'$.country') IN (SELECT value FROM json_each(?)))";bindings.push(JSON.stringify(countryAliases(country)));}
  ...
  return { items: (rows.results || []).map(row => ({ ...row, metadata: metadata(row), entities:JSON.parse(String(row.relations_json||"[]")) })), total: Number(count?.n || 0), page, pages: Math.ceil(Number(count?.n || 0) / limit) };
}
...
export async function literatureEntities(db: ReaderDatabase, url: URL) {
  ...
  const result = await db.prepare(`SELECT n.*,(SELECT count(DISTINCT e.id) FROM library_edition_entities x JOIN library_editions e ON e.id=x.edition_id WHERE x.entity_id=n.id AND ${relationMatch} AND e.fund='literature' AND e.publication_state!='archived') books FROM library_catalog_entities n WHERE ${where} ORDER BY n.name,n.id LIMIT ? OFFSET ?`).bind(kind,entityId,entityId, q, like(normalizeCatalogSearchText(q)), limit, offset).all();
  return { items: (result.results || []).map(row => {const {source_json,...safe}=row;return {...safe,metadata:literatureEntityMetadata(row)};}), total: Number(count?.n || 0), page, pages: Math.ceil(Number(count?.n || 0) / limit) };
}
export async function literatureEntity(db:ReaderDatabase,id:string){const raw=await db.prepare("SELECT * FROM library_catalog_entities WHERE id=? AND COALESCE(json_extract(public_metadata_json,'$.archived'),0)=0").bind(id).first();if(!raw)readerFail("entity_missing","Запис довідника не знайдено.",404);return {id:raw.id,name:raw.name,kind:raw.kind,version:raw.version,metadata:literatureEntityMetadata(raw)};}
...
export async function literatureDashboard(db: ReaderDatabase, url: URL) {
  ...
  return { stats, from, to, activity: activity.results || [], newBooks: books.items.slice(0,10), overdue: overdue.items.slice(0,5) };
}
export async function saveLiteratureReader(db: ReaderDatabase, actor: LibraryActor, input: Input) {
  const hasSubject=Object.hasOwn(input,"subjectPosition"),subject=hasSubject?text(input.subjectPosition,160):"";
  ...
  if(hasSubject){if(subject&&kind!=="teacher")readerFail("reader_subject","Предмет або напрям доступний для вчителя.");statements.push(db.prepare("SELECT CASE WHEN EXISTS(SELECT 1 FROM library_readers WHERE id=? AND linked_teacher_user_id IS NULL) THEN 1 ELSE json('linked_teacher_subject') END").bind(id));}
  statements.push(db.prepare("INSERT INTO reader_profiles(reader_id,display_name,phone,subject_position,updated_at) VALUES(?,'Читач',?,?,?) ON CONFLICT(reader_id) DO UPDATE SET phone=excluded.phone,subject_position=CASE WHEN ? THEN excluded.subject_position ELSE reader_profiles.subject_position END,version=reader_profiles.version+1,updated_at=excluded.updated_at").bind(id,phone,subject,now,Number(hasSubject)));
...
export async function saveLiteratureEntity(db: ReaderDatabase, actor: LibraryActor, input: Input) {
  ...
  const allowed = ["firstName","lastName","nickname","biography","country","dateOfBirth","dateOfDeath","yearBorn","yearDied","website","address","city","location","email","phone","description"];
...
export async function literatureChoices(db:ReaderDatabase,url:URL){
 const kind=url.searchParams.get('kind')||'',copy=new URL(url),{page,limit,offset}=pageInfo(url);let result;
 if(['author','publisher','genre','tag','series'].includes(kind)){result=await literatureEntities(db,copy);return {...result,items:result.items.map((r:any)=>({id:r.id,label:r.name,detail:r.kind==='author'?countryLabel(r.metadata.country):undefined}))};}
 if(kind==='book'||kind==='reader'){copy.searchParams.delete("kind");result=kind==='book'?await literatureBooks(db,copy):await literatureReaders(db,copy);return {...result,items:result.items.map((r:any)=>({id:r.id,label:kind==='book'?r.title:r.full_name,detail:kind==='book'?[r.metadata.author,r.available+' доступно'].filter(Boolean).join(' · '):['№ '+r.member_no,r.class_name||r.source_group_label].filter(Boolean).join(' · ')}))};}
 if(!['class','group','location'].includes(kind))readerFail('choices','Невідомий список.');
 let items: {id:string;label:string;detail?:string}[]=[];
 if(kind==='class'){const r=await db.prepare("SELECT c.id,c.class_name,a.label FROM class_years c JOIN academic_years a ON a.id=c.academic_year_id WHERE c.status IN ('active','planned') ORDER BY a.start_date DESC,c.grade,c.class_name").all();items=(r.results||[]).map((r:any)=>({id:r.id,label:r.class_name,detail:r.label}));}
 if(kind==='location'){const r=await db.prepare("SELECT id,name FROM locations WHERE status='active' AND type='library' ORDER BY sort_order,name").all();items=(r.results||[]).map((r:any)=>({id:r.id,label:r.name}));}
 if(kind==='group'){const r=await db.prepare("SELECT DISTINCT source_group_label name FROM library_readers WHERE source_group_label!='' ORDER BY source_group_label").all();items=(r.results||[]).map(r=>({id:String(r.name),label:String(r.name)}));}
 const tokens=(url.searchParams.get('q')||'').toLocaleLowerCase('uk-UA').split(/\s+/).filter(Boolean);items=items.filter(r=>tokens.every(t=>(r.label+' '+(r.detail||'')).toLocaleLowerCase('uk-UA').includes(t)));
 return {items:items.slice(offset,offset+limit),total:items.length,page,pages:Math.ceil(items.length/limit)};
}
