...
const activeCommunityThread="t.kind!='book'";
...
export async function listReadingThreads(db:ReaderDatabase,identity:ReaderIdentity){await requireReaderCommunity(db,identity.readerId);
  const rows=await db.prepare(`SELECT t.id,t.kind,t.title,t.edition_id,t.owner_reader_id,t.updated_at,m.status AS membership_status,m.role,
    ...
    FROM reading_threads t JOIN reading_memberships m ON m.thread_id=t.id WHERE m.reader_id=? AND m.status IN ('accepted','invited') AND t.status='active' AND ${activeCommunityThread} ORDER BY t.updated_at DESC,t.id LIMIT 100`).bind(identity.readerId,identity.readerId).all();return rows.results||[];}
...
export async function getReadingMessages(db:ReaderDatabase,identity:ReaderIdentity,threadId:string){await requireReaderCommunity(db,identity.readerId);
  const member=await db.prepare(`SELECT 1 ok FROM reading_memberships m JOIN reading_threads t ON t.id=m.thread_id WHERE m.thread_id=? AND m.reader_id=? AND m.status='accepted' AND t.status='active' AND ${activeCommunityThread}`).bind(threadId,identity.readerId).first();if(!member)readerFail("thread_private","Спочатку прийми запрошення до розмови.",403);
  const guard=readerGuard(identity,new Date().toISOString());
  const messages=await db.prepare(`SELECT x.id,x.reader_id,x.body,NULL AS edition_id,x.created_at,p.display_name,NULL AS book_title FROM reading_messages x JOIN reader_profiles p ON p.reader_id=x.reader_id
    WHERE x.thread_id=? AND x.status='visible'
      ...
      AND EXISTS(SELECT 1 FROM reading_memberships live JOIN reading_threads t ON t.id=live.thread_id WHERE live.thread_id=x.thread_id AND live.reader_id=? AND live.status='accepted' AND t.status='active' AND ${activeCommunityThread})
...
export async function createReadingThread(db:ReaderDatabase,identity:ReaderIdentity,input:{requestId:string;kind:"book"|"group"|"direct";title:string;editionId?:string;recipientId?:string}){
  ...
  if(input.kind==="book"||input.editionId)readerFail("librarika_authoritative","Обговорення конкретної книги відкривай із каталогу Librarika; тут доступні читацькі групи та особисті розмови.",410);
  ...
  const existing=input.kind==="direct"?await db.prepare("SELECT id FROM reading_threads WHERE direct_pair_key=? AND status='active'").bind(pair).first():null;
  ...
  if(existing){
    ...
    statements.push(db.prepare("SELECT CASE WHEN EXISTS(SELECT 1 FROM reading_memberships WHERE thread_id=? AND reader_id=? AND status IN ('accepted','invited')) THEN 1 ELSE json('conversation_closed') END").bind(String(existing.id),identity.readerId));
  ...
  statements.push(db.prepare(`INSERT INTO reading_threads(id,kind,title,owner_reader_id,edition_id,direct_pair_key,status,created_at,updated_at)
    VALUES(?,?,?,?,NULL,?,'active',?,?)`).bind(id,input.kind,input.kind==="direct"?"Особиста розмова":input.title.trim(),identity.readerId,pair,now,now));
...
export async function joinBookDiscussion(db:ReaderDatabase,identity:ReaderIdentity,input:{requestId:string;threadId:string}){
  void db;void identity;void input;
  readerFail("librarika_authoritative","Обговорення конкретної книги відкривай із каталогу Librarika.",410);
}
...
export async function sendReadingMessage(db:ReaderDatabase,identity:ReaderIdentity,input:{requestId:string;threadId:string;body:string;editionId?:string|null}){
  if(input.editionId)readerFail("librarika_authoritative","Картки художніх книг надсилай покликанням із Librarika.",410);
  if(typeof input.body!=="string"||input.body.length>4000||!input.body.trim()||!readerResource(input.threadId))readerFail("message_invalid","Напиши повідомлення до 4000 символів.");
  ...
  await readerBatch(db,[readerReceipt(db,identity,input.requestId,"message",replay.hash,result,now),communityGate(db,identity),memberGate(db,identity,input.threadId),directGate(db,input.threadId),
    ...
    db.prepare(`INSERT INTO reading_messages(id,thread_id,reader_id,body,edition_id,status,created_at) VALUES(?,?,?,?,NULL,'visible',?)`).bind(id,input.threadId,identity.readerId,input.body.trim(),now),
