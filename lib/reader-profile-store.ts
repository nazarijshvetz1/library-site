...
export async function getReaderProfile(db:ReaderDatabase,identity:ReaderIdentity){
  const row=await db.prepare(`SELECT r.id,${readerFullNameSql} full_name,r.linked_teacher_user_id,r.member_no,r.kind,r.source_group_label,r.version AS reader_version,
    ${readerDirectoryFieldsSql},p.display_name,p.email,p.photo_key,p.community_enabled,p.notify_loans,p.notify_books,p.version,
  ...
  return {id:row.id,fullName:row.full_name,memberNo:row.member_no,kind:row.kind,classLabel:row.class_label||row.source_group_label||"",classSource:row.class_label?"current":"imported",academicYear:row.academic_year||null,
    displayName:row.display_name,phone:row.phone,email:row.email||"",photoUrl:row.has_photo?"/api/reader/photo?v="+row.profile_version+"&at="+encodeURIComponent(String(row.photo_updated_at||"")):null,subjectPosition:row.subject_position||"",primaryLocation:row.primary_location_id?{id:row.primary_location_id,name:row.primary_location_name||""}:null,teacherProfileVersion:row.teacher_profile_version?Number(row.teacher_profile_version):null,teacherProfileUpdatedAt:row.teacher_profile_updated_at||null,communityEnabled:!!row.community_enabled,notifyLoans:!!row.notify_loans,notifyBooks:false,version:Number(row.version),telegramConnected:row.telegram_status==="active",teacherLinked:!!row.linked_teacher_user_id};
}
export async function updateReaderProfile(db:ReaderDatabase,identity:ReaderIdentity,input:{requestId:string;expectedVersion:number;displayName:string;phone:string;communityEnabled:boolean;email?:string;notifyLoans:boolean;notifyBooks:boolean}){
  if(Object.keys(input).filter(k=>k!=="email").sort().join(",")!==["requestId","expectedVersion","displayName","phone","communityEnabled","notifyLoans","notifyBooks"].sort().join(","))readerFail("profile_fields","Ця форма не може змінювати ім’я, клас або права читача.");
  ...
  if(input.notifyBooks)readerFail("reader_notifications_unavailable","Сповіщення про нові надходження ще не налаштовані.",409);
  if(identity.sessionKind==="teacher"){const canonical=await getReaderProfile(db,identity);if(input.phone!==canonical.phone)readerFail("canonical_teacher_profile","Мобільний номер змінюється в основному кабінеті вчителя й автоматично відображається тут.",409);}
  const displayName=input.displayName.normalize("NFKC").trim().replace(/\s+/g," "),phone=input.phone.trim();
  if(displayName.length<2||displayName.length>180||[...displayName].some(character=>character==="<"||character===">"||character.charCodeAt(0)<32)||(identity.sessionKind!=="teacher"&&(phone.length>30||(phone&&!/^\+?[0-9() .-]{7,30}$/.test(phone))||(phone&&(phone.replace(/\D/g,"").length<7||phone.replace(/\D/g,"").length>15)))))readerFail("profile_invalid","Ім’я для спільноти: 2–180 символів; телефон — до 15 цифр. Телефон можна не вказувати.");
  if(input.email!==undefined&&(typeof input.email!=="string"||input.email.length>254||(input.email.trim()&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(input.email.trim()))))readerFail("profile_email","Перевірте email.");
  ...
  await readerBatch(db,[readerReceipt(db,identity,input.requestId,"profile",replay.hash,result,now),
    db.prepare("UPDATE reader_profiles SET display_name=?,phone=CASE WHEN EXISTS(SELECT 1 FROM library_readers r WHERE r.id=reader_profiles.reader_id AND r.linked_teacher_user_id IS NOT NULL) THEN phone ELSE ? END,email=coalesce(?,email),community_enabled=?,notify_loans_since=CASE WHEN ?=1 AND notify_loans=0 THEN ? WHEN ?=0 THEN NULL ELSE notify_loans_since END,notify_books_since=CASE WHEN ?=1 AND notify_books=0 THEN ? WHEN ?=0 THEN NULL ELSE notify_books_since END,notify_loans=?,notify_books=?,version=version+1,updated_at=? WHERE reader_id=? AND version=?").bind(displayName,phone,input.email?.trim()??null,Number(input.communityEnabled),Number(input.notifyLoans),consentDay,Number(input.notifyLoans),Number(input.notifyBooks),now,Number(input.notifyBooks),Number(input.notifyLoans),Number(input.notifyBooks),now,identity.readerId,input.expectedVersion),requireChanged(db,1),
    db.prepare("UPDATE reader_messages SET delivery_status=CASE WHEN delivery_status='processing' THEN 'uncertain' WHEN ?=1 THEN 'pending' ELSE 'disabled' END,lease_token=NULL,lease_until=NULL,next_attempt_at=? WHERE reader_id=? AND delivery_status IN ('pending','processing','retry','disabled','unavailable') AND (kind!='loan_digest' OR day=?)").bind(Number(input.notifyLoans),now,identity.readerId,consentDay),
...
export async function requestReaderBook(db:ReaderDatabase,identity:ReaderIdentity,input:{requestId:string;editionId:string;note:string;confirmation:string}){
  ...
  await readerBatch(db,[readerReceipt(db,identity,input.requestId,"book_request",replay.hash,result,now),
    db.prepare(`INSERT INTO reader_book_requests(id,reader_id,edition_id,status,note,version,created_at,updated_at)
      VALUES(?,?,(SELECT e.id FROM library_editions e LEFT JOIN materials m ON m.id=e.material_id WHERE e.id=? AND e.fund='literature' AND e.publication_state='published' AND (m.id IS NULL OR m.status='active')),'requested',?,1,?,?)`).bind(id,identity.readerId,input.editionId,input.note.trim(),now,now)]);
  return result;
}
