...
export async function getReaderProfile(db:ReaderDatabase,identity:ReaderIdentity){
  const row=await db.prepare(`SELECT r.id,COALESCE((SELECT full_name FROM users WHERE id=r.linked_teacher_user_id),r.full_name) full_name,r.member_no,r.kind,r.source_group_label,r.version AS reader_version,
    (SELECT photo_version FROM teacher_profiles WHERE teacher_user_id=r.linked_teacher_user_id AND photo_storage_key IS NOT NULL) teacher_photo_version,p.display_name,p.phone,p.email,p.photo_key,p.community_enabled,p.notify_loans,p.notify_books,p.version,
  ...
  return {id:row.id,fullName:row.full_name,memberNo:row.member_no,kind:row.kind,classLabel:row.class_label||row.source_group_label||"",classSource:row.class_label?"current":"imported",academicYear:row.academic_year||null,
    displayName:row.display_name,phone:row.phone,email:row.email||"",photoUrl:row.photo_key?"/api/reader/photo?v="+row.version:identity.sessionKind==="teacher"&&row.teacher_photo_version?"/api/teacher/profile/photo?v="+row.teacher_photo_version:null,communityEnabled:!!row.community_enabled,notifyLoans:false,notifyBooks:false,version:Number(row.version),telegramConnected:row.telegram_status==="active",teacherLinked:identity.sessionKind==="teacher"};
}
