...
export type AssistantToolResult = {
  ...
  cartUpdate?: import("./teacher-cart").AssistantCartUpdate;
};

export type AssistantActionPreview = { id: string; title: string; lines: string[]; expiresAt: string; cartSignature?: string; publicDisplayConsent?: boolean };
...
export function assistantTools(role: AssistantRole): AssistantTool[] {
  ...
  if (role === "teacher") tools.push(
    tool("order_cart", "Переглянути спільний кошик кабінету вчителя. Це ще не надіслане замовлення.", {}),
    tool("update_order_note", "Змінити примітку спільного кошика лише за проханням учителя. Нічого не надсилає.", { details: { type: "object", additionalProperties: false, properties: { note: { type: "string", maxLength: 2000 } }, required: ["note"] } }, ["details"]),
    tool("update_order_cart", "Додати або змінити кількість одного точно обраного видання у спільному кошику. quantity — НОВА загальна кількість, 0 вилучає позицію. Не надсилає бібліотекарю. Спочатку уточни видання і кількість.", { materialId: text("CAT-ID з пошуку"), quantity: { type: "integer", minimum: 0, maximum: 1000 } }, ["materialId", "quantity"]),
    tool("my_visits", "Мої майбутні записи до бібліотеки з ID для перенесення або скасування.", {}),
    tool("my_profile", "Мій профіль, закріплені класи, мобільний номер і стан Telegram-сповіщень. Без чужих профілів.", {}),
    tool("my_acquisitions", "Мої пропозиції придбання, уточнення бібліотекаря й остання відповідь учителя. Не замовлення з фонду.", { query: text("Назва або автор; необов'язково") }),
    tool("my_notifications", "Мої повідомлення бібліотеки; читання тут не змінює позначку прочитано.", { cursor: text("nextCursor попередньої сторінки") }),
    tool("teacher_action_schema", "Точні поля дії: order.submit, order.cancel, visit.update, visit.cancel, acquisition.create, acquisition.reply, profile.phone, notifications.set. Дані не змінює.", { action: text("Назва дії") }, ["action"]),
    tool("prepare_teacher_action", "Підготувати дію лише від імені поточного вчителя. Спершу отримай teacher_action_schema й уточни всі поля. Після цього людина переглядає картку і натискає підтвердження. Усне так не виконує дію.", { action: text("Назва дії"), details: { type: "object", additionalProperties: true, description: "Поля зі схеми; без ідентифікатора вчителя, службових версій і підтвердження" } }, ["action", "details"]),
...
Не сприймай фонові звуки, телевізор, музику або відлуння власного голосу як прохання користувача. Якщо слова нерозбірливі або неясно, чи звертаються до тебе, не вигадуй запит і не готуй дію: коротко попроси повторити. Для гучного динаміка порадь режим «Шумне приміщення», а для складного шуму — «Натисніть, щоб говорити».
Учитель може через власні інструменти керувати спільним кошиком, готувати надсилання й скасування власного замовлення, запис/перенесення/скасування свого відвідування, пропозицію придбання та відповідь на уточнення, зміну свого мобільного номера й Telegram-сповіщень. Показуй власні повідомлення, профіль і класи. Додавання в кошик не означає надсилання; перед order.submit перечитай order_cart. Використовуй ці інструменти без переходів, але не надавай учителю функцій обліку бібліотекаря.
