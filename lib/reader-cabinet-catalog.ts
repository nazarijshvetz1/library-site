...
export async function cabinetPublicBook(db:ReaderDatabase,id:string){await assertReaderEdition(db,id);const row=await db.prepare(`SELECT ${projection},json_extract(e.public_metadata_json,'$.annotation') annotation,json_extract(e.public_metadata_json,'$.description') description FROM ${from} WHERE e.id=?`).bind(id).first();const {description,...card}=row!;return {...cabinetCard(card),annotation:row?.annotation||description||''};}
